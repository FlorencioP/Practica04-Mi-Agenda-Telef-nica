...
function eliminar(id , idmod){
    var con=confirm("esta seguro que quiere eliminar su cuenta");
    if(con === true){
        window.location= '../../controladores/admin/eliminarMOD.php?id='+id+' &idmod='+idmod;
    }
}

function recargarUsu(id){
    editando=false;
    window.location= '../../vista/admin/indexMOD.php?id='+idmod+'&idmod='+id;
}

function modificarTlfMOD(idT,id,idmod){
    editando=false;
    ...
    window.location= '../../controladores/admin/modificarTelefonoMOD.php?idT='+idT+'&id='+id+'&numeroT='+numero+'&tipoT='+tipo+'&operadoraT='+operadora+'&idmod='+idmod;
}

function cambiarRenglon(nodo,idT,id,idmod){
    if (editando == false) {
        ...
        '<td class="tg-y6988"><span style="cursor: pointer" onclick="modificarTlfMOD('+idT+','+id+','+idmod+')">Aceptar</span></td>'+
        '<td class="tg-y6988"><span style="cursor: pointer" onclick="recargarUsu('+id+','+idmod+')">Cancelar</span></td>';
